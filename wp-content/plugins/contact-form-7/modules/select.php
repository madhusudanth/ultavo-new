...
function wpcf7_select_form_tag_handler( $tag ) {
	...
	$categories = $wpdb->get_results("SELECT Category_Name, Category_Type FROM $categories_table_name");
	$products = $wpdb->get_results("SELECT * FROM wp_UPCP_Items");
	if($tag->get_id_option() == "brandscol"){
		$values[] = "";
		$labels[] = "Please Select Brand";
		if($_GET['av']=='1'){
			foreach($categories as $kcat => $vcat){
				if($vcat->Category_Type == 'Audio' || $vcat->Category_Type == 'Video'){					
					$values[] = $vcat->Category_Name;
					$labels[] = $vcat->Category_Name;
				}
			}
		}elseif($_GET['lz']=='1'){
			foreach($categories as $kcat => $vcat){
				if($vcat->Category_Type == 'Audio'){					
					$values[] = $vcat->Category_Name;
					$labels[] = $vcat->Category_Name;
				}
			}
	...
	// for products dropdown on contact us ... add [select* productscol id:productscol class:productscol "p1" "p2" "p3"]  and [hidden cp_id default:get] on form
	
	// elseif($tag->get_id_option() == "productscol"){
	// 	$values[] = "";
	// 	$labels[] = "Please Select Product";
	// 	foreach($products as $kcat => $vcat){
	// 		$values[] = $vcat->Item_Name;
	// 		$labels[] = $vcat->Item_Name;
	// 	}
	// 	$values[] = "Other";
	// 	$labels[] = "Other";
	// }
	...
	foreach ( $values as $key => $value ) {
		...
		// to by default select value from url
		if($_GET['cp_id'] == $value){
			$selected = true;
		}
		if($_GET['cc_brand'] == $value){
			$selected = true;
		}
